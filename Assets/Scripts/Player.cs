...
public class Player : MonoBehaviour
{
    ...
    private void EventTikTok()
    {
        //switch (NameEvent)
        //{
        //    case "Like":  /// Giảm tốc tức thì khi có sự kiện Like
        //        if(currentspeed > 20)
        //            currentspeed = 20;
        //        break;
        //}
    ...
    void Move()
    {
        ...
        collideCooldown -= Time.deltaTime; /// Giảm thời gian chờ của va chạm

        if(!GLIDER_FLY && !JUMP_PANEL && !cancelAddforceDown) /// Thêm lực rơi xuống khi xe không ở trạng thái bay
        {
            if(antiGravity)
                rb.AddRelativeForce(Vector3.down * 5000 * Time.deltaTime, ForceMode.Acceleration);
            else
        ...
        if (antiGravity) /// Thêm lực xuống 
        {
            rb.AddRelativeForce(Physics.gravity * 5, ForceMode.Acceleration);
        } ///

        if(GLIDER_FLY) /// Giảm vận tốc khi đang bay
        {
        ...
        if (GLIDER_FLY) /// Điều chỉnh tốc độ khi đang bay
        {
            if (!Input.GetKey(KeyCode.Space))
            {
        ...
        /// Tự giảm tốc nếu không nhấn space
        if (!Input.GetKey(KeyCode.Space))
        {
        ...
        //glider movements
        if (GLIDER_FLY) /// Di chuyển trong trạng thái bay
        {
            gliderMovements();
        }
        if (JUMP_PANEL)
